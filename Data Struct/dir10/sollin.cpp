...
void sollin()
{
   ...
   while (i < NNODES)  // 각 단계에서 여려개의 간선을 선택해 신장 포리스트를 만든다.
   {
   ...
   i = 0;  // i를 다시 0으로 초기화시킴
   while (i < NNODES)  // 남은 데이터들을 모두 tmp로 이동시킨다.
   {
      while (!PQ[i].empty()) // 우선순위 큐의 원소가 있으면
      {
         Edge e = PQ[i].top();
         tmp.push(e);  // 우선순위 큐에 데이터를 넣는다.
         PQ[i].pop();
      }
      i++;
   }
   ...
   for (int j = 0; j < SIZE; j++)  // 신장 포리스트 트리를 만들기
   {
      Edge e = tmp.top();
      tmp.pop();
